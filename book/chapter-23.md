...
В многопоточных WASM-приложениях (с использованием SharedArrayBuffer) `js_sys::hazard` использует hazard pointers для обеспечения безопасности при доступе к разделяемым объектам . Это гарантирует, что объекты не будут удалены, пока другие потоки обращаются к ним, предотвращая состояния гонки и паники на границе.
