...
```
Функция `spawn_blocking` автоматически управляет пулом воркеров, создавая новые по мере необходимости и адаптируясь к числу поставленных в очередь задач — по духу это напоминает модель `tokio`, где лимит блокирующих потоков по умолчанию составляет 512, а простаивающие потоки завершаются по истечении тайм-аута. Однако точные значения по умолчанию для `async_wasm_task` (лимит числа воркеров, тайм-аут простоя) стоит сверять с актуальной документацией на docs.rs, а не считать зафиксированными — сама библиотека документирует поведение как «автоматическая адаптация», не называя явно конкретные числа в описании крейта.
