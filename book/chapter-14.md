...
Библиотека **web-sys** предоставляет привязки ко всем Web API — от DOM до WebGPU. Каждый вызов web-sys оборачивает JavaScript-вызов, что создаёт накладные расходы на границе Rust↔JS :
...
Одно из ключевых преимуществ Rust в контексте WebAssembly — отсутствие сборщика мусора. Rust использует модель владения (ownership) с подсчётом ссылок и временем жизни, проверяемым на этапе компиляции .
...
Актуальные версии на начало 2026 года :
- `js-sys`: 0.3.95
- `web-sys`: 0.3.95
